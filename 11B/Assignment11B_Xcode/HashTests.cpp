                   ...
                   ChainedHT& chained_table, int& chained_probe_count)
{
    ...
    while(getline(in, line)){
        ...
        for (int i = 0; i < line.length(); i++) {
            if(isalpha(line.at(i)) || isspace(line.at(i))) onlywords += tolower(line.at(i));
            else if (ispunct(line.at(i))) onlywords += " ";
        }
        stringstream lnstream(onlywords);
        while(lnstream >> word){
            word_count++;
                 ...
                 ChainedHT& chained_table, int chained_probe_count)
{
    ...
    double linear_probe_avg = linear_probe_count / (double)word_count;
    double quadratic_probe_avg = quadratic_probe_count / (double)word_count;
    
    double chained_length_avg  = chained_table.avg_chain_length();
    double chained_probe_avg = chained_probe_count / (double)word_count;
